...
callerInstructions: |
  ...
  **Exit code carries the decision.** The run exits `success`
  ...
  inherited automatically from the parent run. Bundled
  implementation workflows also pass `--cwd {{cwd}}` so git
  inspection stays rooted in the checkout being reviewed.
...
tasks:
  - id: orient
    ...
    body: |
      ...
      `assignment-summary.md` row from the implementation run or
      another run in the same run group,
  ...
  - id: plan_coverage
    ...
    body: |
      ...
      **Step 1: determine each task's category.**
      ...
      top of the task's body. Feature plans produced by bundled
      planning assignments such as `plan-feature` and
      `plan-implement-feature` are required to tag every task,
      so this line should be present. Valid values:
      ...
      **Step 2: verify each task based on its category.**
      ...
      For **code-bearing tasks**, additionally:
        ...
        - Look for a `**Done when:**` block in the task body
          (bundled feature plans are required to carry one on
          every code-bearing task). If present,
      ...
      **Step 4b: verify the surface inventory was met.** If
      the plan includes a **Surface Inventory** block in an
      orient task, a feature-context section, or a dedicated
      task such as `feature-plan/surface-inventory`, treat
      that inventory as ground truth for what the contract
      required. The implementer's per-task Notes describe what
      was *done*; the inventory describes what had to be done.
      Walk every inventory entry and confirm the diff covers it
      end-to-end (declaration, consumer, integration test) and
      that any symmetric peers and removal twins are aligned.
      Report:
