  ...
  - id: orient_inputs
    ...
    body: |
      Read the draft plan file plus the planning run's task notes:
          ...
          {{plan_draft}}
          {{agent_runner_cmd}} run status {{planning_run_id}} --output-format json --field tasks

      First, confirm the draft exists and is readable. If it is
      missing or obviously not an agent-runner assignment, mark this
...
---
You are reviewing an agent-runner draft assignment, not the
