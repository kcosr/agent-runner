...
callerInstructions: |
  ...
  If a task is genuinely infeasible, the executor should mark
  it `blocked` with an explanation rather than dropping it.
...
If a task is genuinely infeasible, mark it `blocked` with an
