...
├── run.json        # canonical manifest (see "Run manifest")
...
inspection. `run.json` is the thing you archive, share, or grep; `tasks.md`
is a diagnostic artifact that shows what the markdown looked like at the end
of the run. Users should add `.task-runner/` to `.gitignore`.
...
interface RunManifest {
  ...
  endedAt: string | null;         // null while status === "running"
  ...
  tasksTotal: number;
  backendSessionId: string | null; // claude session id extracted from attempt 1
...
interface AttemptRecord {
  ...
  rawStdout: string;              // full raw backend stdout (stream-json)
  rawStderr: string;
...
**Always-captured raw output**: `rawStdout` is the complete, unfiltered
stdout from the backend for that attempt — for Claude, that means the
stream-json event stream. `rawStderr` is stderr. These can be large, but
they're the only thing that lets you forensically debug a weird failure, so
we capture them unconditionally. The `assistantMessage` field carries the
parsed final message (same thing the user sees in text mode) if you just
...
When the runner re-invokes Claude after an incomplete attempt, it picks up
where it left off (same context, same cache, less token churn) rather than
starting a fresh conversation from zero.
...
- **Extraction fails on attempt 1** (no session ID in any event): the
  adapter returns `sessionId: null`. The runner falls back to fresh
  invocations on retries — the agent loses conversation context but still
  receives the full task state through `tasks.md` and the nudge prompt.
...
- **Session ID storage**: in-memory during the run, then written into
  `run.json` as part of the manifest. The manifest copy is write-only —
  the runner never reads it back. Each `task-runner run` invocation starts
  with no session knowledge and only uses IDs it extracted during the
  current run.
- **No cross-run resumption**: `task-runner` never reads session IDs from
  a prior workspace directory, a config file, or env vars.
...
`AttemptRecord.rawStdout` for forensics, regardless of output mode.
...
- **Cross-run resume**: explicitly out of scope today. If added later it
  would be an opt-in `--resume-run <id>` flag that reads a prior manifest.
- **Manifest compression**: `rawStdout` for long runs can be large. If it
  becomes a problem we could gzip the raw fields or split them into
  per-attempt sidecar files. Not a concern at current scale.
