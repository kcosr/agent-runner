...
└── attempts/
    ├── 01.json           # raw backend stdout/stderr for attempt 1
    ├── 02.json           # raw backend stdout/stderr for attempt 2
    └── ...
```
...
is a diagnostic artifact that shows what the markdown looked like at the
end of the run; `attempts/NN.json` holds the raw, unfiltered backend output
for each attempt (see [Attempt logs](#attempt-logs) below). Users should
add `.task-runner/` to `.gitignore`.
