...
- Added `--task-mode <file|cli>` as a fresh run / init override for assignment task workflow mode. The selected mode is now also lockable via `lockedFields: [taskMode]` and remains frozen on resume.
