...
- The web dashboard settings split now persists only durable board preferences (`hideEmptyColumns`, `collapseFailureStates`, `showArchived`) under `task-runner:web:dashboard-preferences`; transient filters, drawer width/fullscreen state, and per-run drawer tabs reset on full reload instead of carrying across sessions.
