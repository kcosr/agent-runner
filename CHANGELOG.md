...
- Built-in assignments now use semantic task ids (`orient`, `internal_review`, `approval`) instead of numeric prefixes. Array order remains the execution order, while ids stay stable when tasks are inserted or reordered. ([#PR_NUMBER](https://github.com/kcosr/task-runner/pull/PR_NUMBER))
