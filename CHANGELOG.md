...
- The web run detail drawer now shows a `Pending` attempt preview for initialized zero-attempt runs, with `Message`, `Prompt`, and `Output` tabs backed by the new `RunDetail.pendingPrompt` field.
