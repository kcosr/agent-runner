...
test("manifest schemaVersion is 20, captures repo, initializes updatedAt, and starts with no queued resume messages", async () => {
  const dir = tempDir();
  ...
  assert.equal(outcome.manifest.schemaVersion, 20);
  ...
  await withSharedRuntimeEnv(dir, async () => {
    assert.throws(
      ...
      (err) => {
        ...
        assert.match(err.message, /requires schemaVersion 20/);
        return true;
      },
    );
  });
