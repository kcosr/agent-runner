...
test("manifest schemaVersion is 21, captures repo, initializes updatedAt, and starts with no queued resume messages", async () => {
  const dir = tempDir();
  ...
  assert.equal(outcome.manifest.schemaVersion, 21);
  ...
  await withSharedRuntimeEnv(dir, async () => {
    assert.throws(
      ...
      (err) => {
        ...
        assert.match(err.message, /requires schemaVersion 21/);
        return true;
      },
    );
  });
