...
test("built-in shared feature task files are valid config-root named task definitions", () =>
  withEnv({ AGENT_RUNNER_CONFIG_DIR: REPO_ROOT }, () => {
    ...
    assert.match(internalReview.task.body ?? "", /same checkout this run used/);
    assert.doesNotMatch(internalReview.task.body ?? "", /sibling worktree/);
...
test("built-in plan-implement-feature assignment composes shared tasks for a single-run flow", () =>
  withEnv({ AGENT_RUNNER_CONFIG_DIR: REPO_ROOT }, () => {
    ...
    assert.doesNotMatch(internalReview.body ?? "", /sibling worktree/);
...
test("built-in code-review assignment fits single-run feature implementation reviews", () =>
  withEnv({ AGENT_RUNNER_CONFIG_DIR: REPO_ROOT }, () => {
    ...
    assert.ok(orient);
    assert.ok(planCoverage);
    assert.match(loaded.config.callerInstructions ?? "", /checkout being reviewed/);
    assert.doesNotMatch(loaded.config.callerInstructions ?? "", /sibling worktree/);
    assert.match(orient.body ?? "", /implementation run or\s+another run in the same run group/);
    assert.match(planCoverage.body ?? "", /plan-implement-feature/);
    assert.match(planCoverage.body ?? "", /feature-plan\/surface-inventory/);
    assert.doesNotMatch(planCoverage.body ?? "", /plans generated by `plan-feature`/);
  }));
