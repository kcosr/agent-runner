...
test("resolveResumeTarget accepts a well-formed v22 manifest from the unknown bucket", () => {
  const dir = tempDir();
  ...
  assert.equal(resolved.manifest.schemaVersion, 22);
  assert.equal(resolved.manifest.updatedAt, "2026-04-11T16:05:00Z");
});

test("resolveResumeTarget rejects a v22 manifest carrying legacy assignment.workspacePath", () => {
  const dir = tempDir();
