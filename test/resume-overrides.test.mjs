...
test("resolveResumeTarget accepts a well-formed v21 manifest from the unknown bucket", () => {
  const dir = tempDir();
  ...
  assert.equal(resolved.manifest.schemaVersion, 21);
  assert.equal(resolved.manifest.updatedAt, "2026-04-11T16:05:00Z");
});

test("resolveResumeTarget rejects a v21 manifest carrying legacy assignment.workspacePath", () => {
  const dir = tempDir();
