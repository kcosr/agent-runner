 ...
 *
 * Prevents runaway agent-spawning-agent loops where one agent-runner
 * invocation calls a backend that itself shells out to another
 * `agent-runner run`. Without a guard, a misbehaving agent could spin
 * up an unbounded chain of backend processes.
 *
 * Recursion guard mechanism: two env vars travel through every child
 * invocation.
 *
 *   AGENT_RUNNER_CALL_DEPTH       — current depth (0 at the outermost call)
 *   AGENT_RUNNER_MAX_CALL_DEPTH   — hard cap, default 1
 *
 * Backend runtime lineage also travels through child invocations.
 *
 *   AGENT_RUNNER_PARENT_RUN_ID    — lineage parent for nested runs
 *   AGENT_RUNNER_RUN_GROUP_ID     — run group for nested runs
 ...
 * incremented depth so a nested `agent-runner run` inherits it.
 *
 ...
 * `agent-runner run` (depth 1), but that nested invocation refuses to
 ...
 * and explicit. Override with `AGENT_RUNNER_MAX_CALL_DEPTH=N` if you
 * have a real reason for deeper chains.
 */

export const AGENT_RUNNER_CALL_DEPTH_ENV = "AGENT_RUNNER_CALL_DEPTH";
export const AGENT_RUNNER_MAX_CALL_DEPTH_ENV = "AGENT_RUNNER_MAX_CALL_DEPTH";
export const AGENT_RUNNER_PARENT_RUN_ID_ENV = "AGENT_RUNNER_PARENT_RUN_ID";
export const AGENT_RUNNER_RUN_GROUP_ID_ENV = "AGENT_RUNNER_RUN_GROUP_ID";
export const AGENT_RUNNER_RUN_ID_ENV = "AGENT_RUNNER_RUN_ID";
export const AGENT_RUNNER_CWD_ENV = "AGENT_RUNNER_CWD";
...
export class RecursionDepthError extends Error {
  constructor(
    ...
    super(
      `recursion depth ${currentDepth} would exceed max ${maxDepth}\n  this agent-runner invocation is nested ${currentDepth} level(s) deep inside another\n  agent-runner run, which is at or above the safety cap. Set\n  ${AGENT_RUNNER_MAX_CALL_DEPTH_ENV}=N to raise the limit if this is intentional.`,
    );
...
export function readParentRunIdFromEnv(env: NodeJS.ProcessEnv = process.env): string | null {
  const value = env[AGENT_RUNNER_PARENT_RUN_ID_ENV]?.trim();
  return value ? value : null;
}

export function readRunGroupIdFromEnv(env: NodeJS.ProcessEnv = process.env): string | null {
  const value = env[AGENT_RUNNER_RUN_GROUP_ID_ENV]?.trim();
  return value ? value : null;
}
...
export function readRecursionState(env: NodeJS.ProcessEnv = process.env): RecursionState {
  return {
    currentDepth: parseNonNegInt(env[AGENT_RUNNER_CALL_DEPTH_ENV], 0),
    maxDepth: parseNonNegInt(env[AGENT_RUNNER_MAX_CALL_DEPTH_ENV], DEFAULT_MAX_CALL_DEPTH),
  };
}
...
export function buildChildRecursionEnv(
  ...
  const childEnv: Record<string, string> = {
    [AGENT_RUNNER_CALL_DEPTH_ENV]: String(state.currentDepth + 1),
    [AGENT_RUNNER_MAX_CALL_DEPTH_ENV]: String(state.maxDepth),
  };
  if (parentRunId) {
    childEnv[AGENT_RUNNER_PARENT_RUN_ID_ENV] = parentRunId;
  }
  if (runGroupId) {
    childEnv[AGENT_RUNNER_RUN_GROUP_ID_ENV] = runGroupId;
  }
  return childEnv;
}
