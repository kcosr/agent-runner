...
function usage() {
  return [
    "Usage: node scripts/migrate-manifests-v16.mjs [--root <path>] [--repo <name>]... [--file <path>]... [--write]",
    "",
    "Dry-run by default. Use --write to update manifests in place.",
    "Migrates schemaVersion 15 manifests to 16 by adding updatedAt.",
...
function atomicWriteJson(path, value) {
  const tmpDir = mkdtempSync(join(dirname(path), ".migrate-v16-"));
...
function validateBackfillTimestamps(manifest) {
  if (typeof manifest.startedAt !== "string") {
    throw new Error("schemaVersion 15 manifest is missing startedAt string");
  }
  if (manifest.endedAt !== null && typeof manifest.endedAt !== "string") {
    throw new Error("schemaVersion 15 manifest endedAt must be a string or null");
  }
}
...
function migrateManifest(manifest) {
  ...
  if (manifest.schemaVersion === 16) {
    if (typeof manifest.updatedAt !== "string") {
      throw new Error("schemaVersion 16 manifest is missing updatedAt string");
    }
    return { manifest, changed: false };
  }
  if (manifest.schemaVersion !== 15) {
    throw new Error(
      `unsupported schemaVersion ${manifest.schemaVersion}; migrate to schemaVersion 15 first`,
    );
  }
  if (manifest.status === "running") {
    throw new Error(
      "schemaVersion 15 manifest is running; stop the run before migrating this workspace",
    );
  }
  validateBackfillTimestamps(manifest);
  const { updatedAt: _updatedAt, ...rest } = manifest;
  return {
    manifest: {
      ...rest,
      schemaVersion: 16,
      updatedAt: manifest.endedAt ?? manifest.startedAt,
    },
    changed: true,
  };
}
...
function migrateManifestFile(record, write, stats) {
  ...
  if (!changed) {
    process.stdout.write(`OK    ${record.label}: already canonical schemaVersion 16\n`);
    return;
  }
  stats.migrated += 1;
  if (write) {
    atomicWriteJson(record.path, after);
    process.stdout.write(`WRITE ${record.label}: promoted to schemaVersion 16\n`);
  } else {
    process.stdout.write(`DRY   ${record.label}: would promote to schemaVersion 16\n`);
  }
}
