...
function usage() {
  return [
    "Usage: node scripts/migrate-manifests-v6.mjs [--root <path>] [--write]",
    "",
...
function migrateManifest(parsed) {
  ...
  if (parsed.schemaVersion === 6) {
    if (!Array.isArray(parsed.attachments)) {
      return { kind: "error", reason: "schemaVersion 6 manifest is missing attachments array" };
    }
  ...
  if ("attachments" in parsed && !Array.isArray(parsed.attachments)) {
    return { kind: "error", reason: "attachments must be an array when present" };
  }
...
function main() {
  ...
  for (const manifestPath of manifests.sort()) {
    ...
    if (result.kind === "noop") {
      noopCount += 1;
      process.stdout.write(`OK    ${manifestPath}: already schemaVersion 6 with attachments\n`);
      continue;
    }
    ...
    if (options.write) {
      ...
      process.stdout.write(`WRITE ${manifestPath}: updated to schemaVersion 6 with attachments\n`);
    } else {
      process.stdout.write(
        `DRY   ${manifestPath}: would update to schemaVersion 6 with attachments\n`,
      );
    }
  }
