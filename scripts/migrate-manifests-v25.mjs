...
function usage() {
  return [
    "Usage: node scripts/migrate-manifests-v25.mjs [--root <path>] [--repo <name>]... [--file <path>]... [--write]",
    "",
    "Dry-run by default. Use --write to update manifests in place.",
    "Migrates schemaVersion 24 manifests to 25 by adding parent completion notification state",
    "and source metadata defaults for queued resume messages and sessions.",
...
function atomicWriteJson(path, value) {
  const tmpDir = mkdtempSync(join(dirname(path), ".migrate-v25-"));
...
function isNullableParentCompletionResumeSource(value) {
  return (
    value === null ||
    (isObjectRecord(value) &&
      value.kind === "parent_completion_notification" &&
  ...
  return (
    ...
    value.every(
      (message) =>
        ...
        isNullableParentCompletionResumeSource(message.source),
    )
  );
}
...
function isSessions(value) {
  return (
    ...
    value.every(
      (session) =>
        isObjectRecord(session) && isNullableParentCompletionResumeSource(session.resumeSource),
    )
  );
}

function validateCanonicalV25(manifest) {
  if (!Array.isArray(manifest.parentCompletionNotifications)) {
    throw new Error("schemaVersion 25 manifest is missing parentCompletionNotifications array");
  }
  if (!isQueuedResumeMessages(manifest.queuedResumeMessages)) {
    throw new Error("schemaVersion 25 manifest queuedResumeMessages are missing source metadata");
  }
  if (!isSessions(manifest.sessions)) {
    throw new Error("schemaVersion 25 manifest sessions are missing resumeSource metadata");
  }
}
...
function migrateManifest(manifest) {
  ...
  if (manifest.schemaVersion === 25) {
    validateCanonicalV25(manifest);
    return { manifest, changed: false };
  }
  if (manifest.schemaVersion !== 24) {
    throw new Error(
      `unsupported schemaVersion ${manifest.schemaVersion}; migrate to schemaVersion 24 first`,
    );
  }
  if (manifest.status === "running") {
    throw new Error(
      "schemaVersion 24 manifest is running; stop the run before migrating this workspace",
    );
  }
  if (!Array.isArray(manifest.queuedResumeMessages)) {
    throw new Error("schemaVersion 24 manifest is missing queuedResumeMessages array");
  }
  if (!Array.isArray(manifest.sessions)) {
    throw new Error("schemaVersion 24 manifest is missing sessions array");
  }
...
function migrateManifestFile(record, write, stats) {
  ...
  if (!changed) {
    process.stdout.write(`OK    ${record.label}: already canonical schemaVersion 25\n`);
    return;
  }
  stats.migrated += 1;
  if (write) {
    atomicWriteJson(record.path, after);
    process.stdout.write(`WRITE ${record.label}: promoted to schemaVersion 25\n`);
  } else {
    process.stdout.write(`DRY   ${record.label}: would promote to schemaVersion 25\n`);
  }
}
