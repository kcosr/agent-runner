...
function usage() {
  return [
    "Usage: node scripts/migrate-manifests-v17.mjs [--root <path>] [--repo <name>]... [--file <path>]... [--write]",
    "",
    "Dry-run by default. Use --write to update manifests in place.",
    "Migrates schemaVersion 16 manifests to 17 by renaming backendSpecific to backendConfig.",
...
function atomicWriteJson(path, value) {
  const tmpDir = mkdtempSync(join(dirname(path), ".migrate-v17-"));
...
function selectedBackendConfig(backend, backendSpecific, fieldPath) {
  if (backend !== "codex") {
    return undefined;
  }
  ...
  if (!isObjectRecord(backendSpecific)) {
    throw new Error(`schemaVersion 16 manifest ${fieldPath} must be an object when present`);
  }
  const codexConfig = backendSpecific.codex;
  if (codexConfig === undefined) {
    return undefined;
  }
  if (!isObjectRecord(codexConfig)) {
    throw new Error(`schemaVersion 16 manifest ${fieldPath}.codex must be an object`);
  }
  return structuredClone(codexConfig);
}

function validateCanonicalV17(manifest) {
  if ("backendSpecific" in manifest) {
    throw new Error("schemaVersion 17 manifest still contains backendSpecific");
  }
  if (!isObjectRecord(manifest.resetSeed)) {
    throw new Error("schemaVersion 17 manifest is missing resetSeed object");
  }
  if ("backendSpecific" in manifest.resetSeed) {
    throw new Error("schemaVersion 17 manifest still contains resetSeed.backendSpecific");
  }
}
...
function migrateManifest(manifest) {
  ...
  if (manifest.schemaVersion === 17) {
    validateCanonicalV17(manifest);
    return { manifest, changed: false };
  }
  if (manifest.schemaVersion !== 16) {
    throw new Error(
      `unsupported schemaVersion ${manifest.schemaVersion}; migrate to schemaVersion 16 first`,
    );
  }
  if (manifest.status === "running") {
    throw new Error(
      "schemaVersion 16 manifest is running; stop the run before migrating this workspace",
    );
  }
  if (!isObjectRecord(manifest.resetSeed)) {
    throw new Error("schemaVersion 16 manifest is missing resetSeed object");
  }
  if ("backendConfig" in manifest) {
    throw new Error("schemaVersion 16 manifest already contains backendConfig");
  }
  if ("backendConfig" in manifest.resetSeed) {
    throw new Error("schemaVersion 16 manifest already contains resetSeed.backendConfig");
  }
  ...
  const { backendSpecific: resetSeedBackendSpecific, ...resetSeedRest } = resetSeedBefore;
  const backendConfig = selectedBackendConfig(manifest.backend, backendSpecific, "backendSpecific");
  const resetSeedBackendConfig = selectedBackendConfig(
    resetSeedBefore.backend,
    resetSeedBackendSpecific,
    "resetSeed.backendSpecific",
  );
  ...
  return {
    manifest: {
      ...
      resetSeed: {
        ...resetSeedRest,
        ...(resetSeedBackendConfig === undefined ? {} : { backendConfig: resetSeedBackendConfig }),
      },
    },
    changed: true,
  };
...
function migrateManifestFile(record, write, stats) {
  ...
  if (!changed) {
    process.stdout.write(`OK    ${record.label}: already canonical schemaVersion 17\n`);
    return;
  }
  stats.migrated += 1;
  if (write) {
    atomicWriteJson(record.path, after);
    process.stdout.write(`WRITE ${record.label}: promoted to schemaVersion 17\n`);
  } else {
    process.stdout.write(`DRY   ${record.label}: would promote to schemaVersion 17\n`);
  }
}
